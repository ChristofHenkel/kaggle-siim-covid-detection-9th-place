...
def batch_to_device(batch, device):
    batch_dict = {key: batch[key].to(device) for key in ["target", "input", "study_idx"]}
...
def custom_collate(batch):
    ...
    for key in ["target", "input", "study_idx"]:
...
class CustomDataset(Dataset):
    ...
    def halfmix(self, image, label, boxes):
        ...
        if np.random.random() <= 0.5:
            image[:, half:, :] = rnd_img[:, half:, :]
            boxes_left = boxes
            boxes_right = rnd_boxes
        else:
            image[:, :half, :] = rnd_img[:, :half, :]
            boxes_left = rnd_boxes
            boxes_right = boxes

        result_boxes = []

        for box in boxes_left:
            # box[2] = box[2].clip(0,half)
            x_min, y_min, x_max, y_max = map(int, box[:4])
            if x_min <= half and x_max <= half:
                result_boxes.append(box)

        for box in boxes_right:
            # box[0] = box[0].clip(half, max_size)
            x_min, y_min, x_max, y_max = map(int, box[:4])
            if x_min >= half and x_max >= half:
                result_boxes.append(box)
