...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.backbone, config = get_backbone(cfg)

        self.conv_head = ConvHead(config)
        ...
        self.head = nn.Linear(backbone_out, cfg.n_classes)
    ...
    def forward(self, batch):
        ...
        x_out = self.backbone.model.backbone(images)
        x = self.backbone.model.fpn(x_out)
        class_out = self.backbone.model.class_net(x)
        box_out = self.backbone.model.box_net(x)
        ...
        x_classes = self.head(x_classes)

        if self.backbone.anchor_labeler is None:
            # target should contain pre-computed anchor labels if labeler not present in bench
            assert "label_num_positives" in target
            cls_targets = [target[f"label_cls_{l}"] for l in range(self.backbone.num_levels)]
            box_targets = [target[f"label_bbox_{l}"] for l in range(self.backbone.num_levels)]
            num_positives = target["label_num_positives"]
        else:
            (
                cls_targets,
                box_targets,
                num_positives,
            ) = self.backbone.anchor_labeler.batch_label_anchors(target["bbox"], target["cls"])

        loss, class_loss, box_loss = self.backbone.loss_fn(
            class_out, box_out, cls_targets, box_targets, num_positives
        )
